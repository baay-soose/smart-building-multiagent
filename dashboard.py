...
/* ── CARD ITEM (flux + décisions) — classes unifiées ── */
.card-item {
...
.ci-desc   { font-size:0.73rem;color:#64748b;line-height:1.5;margin-bottom:3px; }
.ci-action { font-size:0.69rem;color:#94a3b8;font-style:italic;line-height:1.4; }
...
.stButton > button {
    ...
    color:#64748b !important;transition:all .15s !important; min-width:0 !important; idth:100% !important; 
}
...
def get_mqtt_client():
    ...
    client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id="dashboard-sb")
...
def main():
    st.set_page_config(page_title="Smart Building",
    ...
    norm_zones = sum(1 for z in ZONES if z not in zones_with_anomaly and state["sensor_status"][z] == "running")
    ...
    for col, val, label, cls in zip(
        st.columns(5),
        [f"{active_zones}/4", str(total_anom), str(fire_count), str(high_count), str(norm_zones)],
    ...
    for zone, col in zip(ZONES, st.columns(4)):
        with col:
            with state["lock"]:
            ...
            with st.container(border=True):
                ...
                with th:
                    st.markdown(
                        ...
                        f'<span style="font-size:.9rem;font-weight:700;color:#1e293b">{ZONE_LABELS[zone]}</span>'
        ...
        if anomalies_all:
            ...
            for a in anomalies_all[:25]:
                ...
                html += (
                    ...
                    f'<div class="ci-zone">{ZONE_LABELS.get(a["location"], a["location"])}</div>'
        ...
        if decisions:
            ...
            for d in decisions[:15]:
                ...
                html += (
                    ...
                    f'<div class="ci-zone">{ZONE_LABELS.get(d["location"], d["location"])}</div>'
